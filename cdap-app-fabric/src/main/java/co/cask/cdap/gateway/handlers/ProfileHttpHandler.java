...
public class ProfileHttpHandler extends AbstractHttpHandler {
  ...
  /**
   * List runs that the profile has been assigned to, either as the manual run profile or as a schedule profile.
   */
  @POST
  @Path("/namespaces/{namespace-id}/profiles/{profile-name}/programs")
  public void getAssignedPrograms(HttpRequest request, HttpResponder responder,
                                  @PathParam("namespace-id") String namespaceId,
                                  @PathParam("name") String profileName) throws NotFoundException {
    // TODO: implement the method
  }
